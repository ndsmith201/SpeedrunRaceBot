...
def load_start_options(path: Path) -> list[StartOption]:
    ...
    if not isinstance(data, dict):
        raise TypeError("Race options YAML must contain a top-level mapping.")
    ...
    for raw_option in raw_options:
        if not isinstance(raw_option, dict):
            raise TypeError("Every race_start_options entry must define name and value.")
        ...
        if len(set(normalized_values)) != len(normalized_values) or any(
            ...
            raise RuntimeError(
                "Race start option values must be unique and 100 characters or fewer."
            )
